...
# CRC TABLES
    ...
    def takePicture(self):
        self._sendCmd(0x68, self.TELLO_CMD_TAKE_PICTURE,None)
###############################################################################
# utility functions
###############################################################################
        ...
        while not stop_event.is_set():
            try:
                ...
                if cmdID == self.TELLO_CMD_CONN_ACK:
                    ...
                    print(fileType, fileSize, fileID)
                    self.__setattr__("file"+fileID, 
                            {"size": fileSize,
                            "receivedBytes": 0,
                            "imageBytes": ByteBuffer.allocate(fileSize),
                            "pieces": {}
                            })
                    ...
                    chunkLen = payload.get_ULInt16()
                    print(fileID, filePiece, fileChunk, chunkLen) 
                    out = {}
                    try:
                        local_file = self.__getattribute__("file"+fileID)
